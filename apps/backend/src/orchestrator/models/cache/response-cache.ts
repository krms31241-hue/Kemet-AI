...
import type { ChatRequest } from "../providers/provider.types.js";
import type { CacheEntry, CacheOptions, CacheStats } from "./cache.types.js";
 ...
 */
export function buildCacheKey(providerId: string, model: string, request: ChatRequest): string {
  const normalized = {
    providerId,
    model,
...
export class ResponseCache<T> {
  ...
  public set(key: string, value: T, ttlMsOverride?: number): void {
    const now = Date.now();
