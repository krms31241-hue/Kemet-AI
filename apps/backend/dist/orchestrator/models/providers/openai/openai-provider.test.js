...
function createManualStream(parts) {
...
describe("OpenAIProvider.initialize()", () => {
    test("succeeds when the registration id matches the provider id", async () => {
        ...
        await assert.doesNotReject(() => provider.initialize({
            id: "openai",
            name: "OpenAI",
            enabled: true,
            priority: 0,
...
describe("OpenAIProvider.health()", () => {
    test("returns true when the /models endpoint responds successfully", async () => {
        const { fetchStub, calls } = createMockFetch(() => new Response("{}", { status: 200 }));
        const provider = new OpenAIProvider({
            apiKey: "test-key",
            fetchImpl: fetchStub,
    ...
    test("returns false when the upstream request fails", async () => {
        const { fetchStub } = createMockFetch(() => new Response('{"error":{"message":"unauthorized"}}', { status: 401 }));
        const provider = new OpenAIProvider({
            apiKey: "bad-key",
            fetchImpl: fetchStub,
            retryPolicy: new NoRetryPolicy(),
